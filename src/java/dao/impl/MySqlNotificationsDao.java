...
public class MySqlNotificationsDao implements NotificationsDao {
    ...
    public List<Notifications> getNotifications(String username) {
        try {
            ...
            pst = conn.prepareStatement("SELECT * FROM notifications WHERE (usernameB=? and notification_type!='messages' and date_created<NOW()) order by date_created DESC");
    ...
    public boolean insertNotification(Notifications notification) {
        try {
            ...
            Connection conn = MySqlDaoFactory.createConnection();
            pst = conn.prepareStatement("INSERT INTO notifications(usernameA, usernameB, notification_type, message, date_created, referred_id, isReadA, isReadB ) VALUES(?,?,?,?,?,?,?,?)");
    ...
    public int getUnreadNotifications(String username) {
        try {
            ...
            int count = 0;
            pst = conn.prepareStatement("SELECT COUNT(*) FROM notifications WHERE (usernameB=? and isReadB='N' and notification_type!='messages' and date_created<NOW())");
            pst.setString(1, username);
    ...
    public int getLastInsertedNotificationID() {
        try {
            ...
            int count = 0;
            pst = conn.prepareStatement("select MAX(idnotifications) from notifications");
            ...
            Logger.getLogger(MySqlNotificationsDao.class.getName()).log(Level.SEVERE, null, ex);
        }
        return 0;
    }
    ...
    public boolean deleteNotification(int id) {
        try {
            ...
            PreparedStatement pst;
            Connection conn = MySqlDaoFactory.createConnection();
            pst = conn.prepareStatement("DELETE FROM notifications  WHERE"
                    + " idnotifications=?");
            pst.setInt(1, id);
    ...
    public boolean updateNotificationReminder(Notifications notifications){
        try {
            ...
            PreparedStatement pst;
            Connection conn = MySqlDaoFactory.createConnection();
            pst = conn.prepareStatement("UPDATE notifications SET usernameA=?, usernameB=?, message=?, date_created=?, isReadA=?,isReadB=?"
                    + "WHERE (referred_id=? and notification_type=?)");
            pst.setString(1, notifications.getUsernameA());
            pst.setString(2, notifications.getUsernameB());
            
            pst.setString(3, notifications.getMessage());
            ...
            pst.setString(5, notifications.getIsReadA());
            pst.setString(6, notifications.getIsReadB());
            pst.setInt(7, notifications.getReferred_id());
            pst.setString(8, notifications.getNotification_type());
